...
- 答案计划：展示本轮问题被拆成了哪些攻略分面，以及必须覆盖的回答栏目。
- 历史会话继续追问。
- 本地会话删除和清空。
- 运行环境、模型、知识库版本等诊断信息。
...
- **问题分面检索**：复杂问题会拆成多个检索面，例如 Boss 机制、深渊上下半、推荐队伍、装备词条、任务进度、探索路线和活动奖励，避免只用一个宽泛关键词检索。
- **答案计划**：攻略类回答会按问题类型生成必答栏目。例如深渊问题覆盖“敌人和上下半 / 关键机制 / 推荐队伍 / 打法步骤”，任务进度问题覆盖“可确认信息 / 合理推断 / 剩余流程 / 下一步”。
...
当问题包含多个目标，例如“这两个 Boss 机制是什么、怎么打、这期深渊怎么配队”，以太会先拆成机制、打法、队伍、上下半等分面，再分别检索和回答。这样能避免只给一句泛泛结论，也方便在后台查看本轮答案计划和采用的来源。
